...
- **PriceHistory:** this model is used to store a ebay's product previous price, in order to generate a price graph
...
- **Normal AppUser:** this AppUser has "is_staff = False". He can be created by using the standard sign in procedure.
    ...
    - **Manage it's own observations**
    - **Manage it's own notifications**
...
- **Notifications endpoints:**
    ...
    App user token is used to retrieve the user id required to understand which user notification must be
     returned.
...
- **Observations endpoints:**
    - `POST /create_observation`: this endpoint is used to generate an ObservedProduct for a certain app user.
       ...
       An example of the required body for this endpoint is shown down below:
        ...
        ```
    - `GET /get_all_observation`: this endpoint is used by an admin to obtain all user's ObservedProduct.      
...
One of the aims of our project is to notify an user when the price of it's desired product drops below a threshold
 price.
In order to reach our goal we have to check periodically the price of each product inside our database, using ebay API.
...
Another core task of our project is to obtain a list of recommended products for each AppUser: to obtain this list of
 products we implemented a recommendation system that requires all the previous and new observations generated by
  the users.
  
To do that, we exposed a bunch of "communication" endpoints: 
...
- `POST /communication/add_recommendation`: this endpoint is used to receive a list of new recommended products for
 certain AppUsers from our recommendation system. 
