...
## 5. Notifications
When a price of an ebay product drops below an user's observation threshold, a notification is generated by our Django server.
In order to retrieve a notification our xamarin frontend implements a periodic function inside the application root page, to request every minute all the new notification for the logged user.

To handle notifications correctly in IoS and Android, we've created a public interface called `INotificationManager`, which contains the following elements:
- `Attribute NotificationReceived`: this attribute is an event which is associated with a default EventHandler delegate. This event will be raised in the methods expected by INotificationManager interface.
- `Method Initialize()`: this method is used to initialize the notification handler.
- `Method SendNotification(string title, string message, DateTime? notifyTime = null)`: this method is used to send a notification of given title and message. Thanks to this method is possible to set a notification time (a standard notification is immediately delivered to the logged user by default).
- `Method ReceiveNotification`: this method is used to receive a notification of given title and message.

Notifications are showed to the user as shown down below:

![Controller](./Images/Notifications/Notification.jpg)
